...
if __name__ == '__main__':
 ...
 The algorithm groups data into k clusters, each is represent by its cluster centroid, which is chosen randomly in our showcase, as shown in the following figure.\n
    '''
    mlcat(title="K-means algorithm showcase demo",
          text=introduction, delim="=", begin="", end="")
    fig, ax = plt.subplots()

    # The first showcase
    kmeans = KMeans(k=3, samples=3 * 100,
                    centers=[[1, 1], [1, -1], [-1, -1]], repeat_times=1)
    ax.set_xlabel("age")
 ...
 The algorithm would stop at an optimal point where repeating the 2 steps would not increase any performance, or the centers would stop moving.\n
    '''
    mlcat(title="K-means algorithm showcase demo",
          text=introduction_steps, delim="=", begin="", end="")
    # animation
    ...
    fig, ax = plt.subplots()
    mlcat(title="K-means algorithm showcase demo",
          text="Here is another example dataset for clustering, which has more separate data points: \n", delim="=", begin="", end="\n")
    kmeans = KMeans(k=3, samples=3 * 100, cluster_std=0.8,
                    centers=[[0, 5], [4, -3], [-4, -3]], repeat_times=1)
    kmeans.set_ax(ax)
    # animation
    ani = animation.FuncAnimation(
        fig, update, frames=40, fargs=(kmeans,), interval=500)
    writer = animation.FFMpegWriter(
        fps=30, metadata=dict(artist='Me'), bitrate=1800)
    ...
    fig, ax = plt.subplots()
    mlcat(title="K-means algorithm showcase demo",
          text="Applying the algorithm on the well known iris dataset: \n", delim="=", begin="", end="\n")
