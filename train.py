...
parser.add_argument("path", nargs="?", help="file(s), folder, or url to input, in the format of csv", type = argparse.FileType('r'), default = sys.stdin)
parser.add_argument("-o", "--output", nargs="?",help="filepath for output, or will output to stdout", type = argparse.FileType('w'), default = sys.stdout)
parser.add_argument("-v", "--visualise", help="with this option, will popout an video for visualisation", action="store_true")
parser.add_argument("-s", "--savemovie", help="the path for movie save path", type = pathlib.Path, default = join_path("examples/train.mp4"))
...
if args.visualise:
    mlpreview(save_movie,begin="",msg=header) # the mlpreview will call print(begin+prompt), which includes a '\n'
